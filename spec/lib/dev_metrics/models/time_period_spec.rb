...
RSpec.describe DevMetrics::Models::TimePeriod do
  subject(:time_period) { described_class.new(start_date, end_date) }
  ...
  describe '#initialize' do
    it 'sets start and end dates' do
      expect(time_period.start_date).to be_within(1).of(start_date)
      expect(time_period.end_date).to be_within(1).of(end_date)
    end
    ...
    context 'with string dates' do
      ...
      it 'parses string dates correctly' do
        expect(time_period.start_date).to be_a(Time)
        expect(time_period.end_date).to be_a(Time)
      end
    end
    ...
    context 'with integer (days ago)' do
      ...
      it 'converts days ago to time' do
        expect(time_period.start_date).to be < Time.now
        expect(time_period.end_date).to be_within(60).of(Time.now)
      end
    end
  ...
  describe '#duration_days' do
    it 'calculates correct duration' do
      ...
      expect(time_period.duration_days).to eq(expected_days)
    end
  end
  ...
  describe '#contains?' do
    ...
    it 'returns true for date within period' do
      expect(time_period.contains?(middle_date)).to be true
    end

    it 'returns false for date before period' do
      expect(time_period.contains?(before_date)).to be false
    end

    it 'returns false for date after period' do
      expect(time_period.contains?(after_date)).to be false
    end

    it 'handles string dates' do
      middle_string = middle_date.strftime('%Y-%m-%d %H:%M:%S')
      expect(time_period.contains?(middle_string)).to be true
    end
  end

  describe '#git_since_format' do
    it 'returns date in Git format' do
      expect(time_period.git_since_format).to match(/\d{4}-\d{2}-\d{2}/)
    end
  end

  describe '#git_until_format' do
    it 'returns date in Git format' do
      expect(time_period.git_until_format).to match(/\d{4}-\d{2}-\d{2}/)
    end
  end

  describe '#to_h' do
    it 'includes all relevant information' do
      hash = time_period.to_h
      expect(hash[:start_date]).to eq(time_period.start_date)
      expect(hash[:end_date]).to eq(time_period.end_date)
      expect(hash[:duration_days]).to eq(time_period.duration_days)
    end
  end

  describe '#to_s' do
    it 'formats dates nicely' do
      string_repr = time_period.to_s
      expect(string_repr).to include(time_period.start_date.strftime('%Y-%m-%d'))
      expect(string_repr).to include(time_period.end_date.strftime('%Y-%m-%d'))
  ...
  describe '#==' do
    ...
    it 'returns true for same periods' do
      expect(time_period).to eq(same_period)
    end

    it 'returns false for different periods' do
      expect(time_period).not_to eq(different_period)
    end
  end
end
