  ...
  module CLI
    ...
    class RepositorySelector
      ...
      def find_repositories(recursive: false)
        ...
        if recursive
          repositories.concat(find_nested_repositories)
        end
      ...
      def parse_selection(input, max_count)
        ...
        input.split(',').each do |part|
          ...
          if part.include?('-')
            ...
            if valid_range?(start_num, end_num, max_count)
              indices.concat((start_num - 1)..(end_num - 1))
            end
          else
            # Handle single numbers
            num = part.to_i
            if valid_number?(num, max_count)
              indices << num - 1
            end
          end
        end
